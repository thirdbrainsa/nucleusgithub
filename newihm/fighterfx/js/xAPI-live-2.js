...
var nb10000=balancetest/200000;
...
vlot=vlot.toFixed(2);
if (localStorage.getItem("stockenable")==1)
 {
	 vlot=vlot*10;
	 if (vlot<1)
	 {
 ...
 if (localStorage.getItem("forexenable")==1)
 {
	if (vlot<0.01)
		{
...
function closePosition()
{
 ...
 function parseGetAllTradeProfitV1(returnData,ordersearch) {
				
				//
					// EQUITY TEST
					var equity=localStorage.getItem("Equityrunning");
					var balance=localStorage.getItem("BalanceCurrent");
					
					var ratio=parseFloat(equity)/parseFloat(balance);
					...
					if (ratio>1.02)
					{
						...
						localStorage.setItem("megastop",1);
						
					}
					
				
						
					//alert(equity+"/"+balance);
				...
				else
				{
					...
					if (totalTrade==0)
					{
						...
						localStorage.setItem("megastop",0);
					}
					else
					{
...
function parseGetAllTradeProfitV2(returnData,ordersearch) {
...
function parseGetAllTradeProfitV3(returnData,ordersearch) {
 ...
 function parseGetAllTrade(returnData,ordersearch) {
				...
				else
				{
					...
					if (stringPos!="")
						{
					//document.getElementById("position").innerHTML=stringPos;
							localStorage.setItem("listposition",stringPos);
						}	
					}
					else
					{
						//localStorage.setItem("listposition","NO TRADES...");
...
function parseHistory(returnData,ordersearch) {
				
				
				if (returnData.length==0)
				{
					document.getElementById("position").innerHTML="There is no trade in your history during the last month";
					localStorage.removeItem("type");
					localStorage.removeItem("position");
					localStorage.removeItem("profit");
				}
				else
				{
					...
					{
					var stringPos="<table id='pos' width=952 cellpadding=0 cellspacing=0 border=0>";
					stringPos=stringPos+'<thead><tr><th>ID</th><th>Open Time</th><th>Close Time</th><th>Symbol</th><th>Command</th><th>Volume</th><th>Open Price</th><th>Close Price</th><th>Order</th><th>Com.</th><th>Swap</th><th>Profit</th></tr></thead>';
					...
					var swap=returnData[i].storage;
					var openTime=returnData[i].open_time;
					var closeTime=returnData[i].close_time;
					if (ordersearch==order2)
							{
					localStorage.setItem("order",order);
					localStorage.setItem("profit",profit);
					...
					if (order2!=0)
					{
					stringPos=stringPos+'<tr><td>'+ii+'</td></td><td>'+openTimeDate+'</td><td>'+closeTimeDate+'</td><td>'+symbol+'</td><td>'+Stype+'</td><td>'+volume+'</td><td>'+openPrice+'</td><td>'+close_price+'</td><td>'+order2+'</td><td>'+com.toFixed(2)+'</td><td>'+swap.toFixed(2)+'</td><td>'+profit.toFixed(2)+'</td></tr>';
					}				
						};
					...
					if (stringPos!="")
						{
									document.getElementById("position").innerHTML=stringPos;
									localStorage.setItem("historylocal",stringPos);
						}	
					}
			}
...
//closePosition();
	...
	getPrecision=localStorage.getItem("digit");
    if (getPrecision==5)
  {
    var newsl=parseFloat(ask)-parseFloat(stoploss)/10000;
    var newtp=parseFloat(ask)+parseFloat(takeprofit)/10000;
	
	newsl=newsl.toFixed(5);
	newtp=newtp.toFixed(5);
  }
  if (getPrecision==4)
  {
    var newsl=parseFloat(ask)-parseFloat(stoploss)/1000;
    var newtp=parseFloat(ask)+parseFloat(takeprofit)/1000;
	
	newsl=newsl.toFixed(4);
	newtp=newtp.toFixed(4);
  }
 if (getPrecision==3)
  {
    var newsl=parseFloat(ask)-parseFloat(stoploss)/100;
    var newtp=parseFloat(ask)+parseFloat(takeprofit)/100;
	
	newsl=newsl.toFixed(3);
	newtp=newtp.toFixed(3);
  }
   if (getPrecision==2)
  {
    var newsl=parseFloat(ask)-parseFloat(stoploss)/10;
    var newtp=parseFloat(ask)+parseFloat(takeprofit)/10;
	...
	var arguments = {};
	     arguments.tradeTransInfo = {};
				  ...
				  arguments.tradeTransInfo.ie_deviation=0;
				  arguments.tradeTransInfo.sl=newsl;
				  arguments.tradeTransInfo.tp=newtp;
   ...
   function openSellStrength()
{
 localStorage.setItem("waitforanswer",1);
   ...
   var symbolCurrent=localStorage.getItem("symbolCurrent");
    ...
    var getPrecision=localStorage.getItem("digit");
	
    if (getPrecision==5)
	{
    var newsl=parseFloat(bid)+parseFloat(stoploss)/10000;
    var newtp=parseFloat(bid)-parseFloat(takeprofit)/10000;
	...
	newtp=newtp.toFixed(5);
     ...
     if (getPrecision==3)
	{
    var newsl=parseFloat(bid)+parseFloat(stoploss)/100;
    var newtp=parseFloat(bid)-parseFloat(takeprofit)/100;
	...
	newtp=newtp.toFixed(3);
       ...
       if (getPrecision==2)
	{
    var newsl=parseFloat(bid)+parseFloat(stoploss)/10;
    var newtp=parseFloat(bid)-parseFloat(takeprofit)/10;
	...
	newtp=newtp.toFixed(2);
          ...
          if (getPrecision==4)
	{
    var newsl=parseFloat(bid)+parseFloat(stoploss)/1000;
    var newtp=parseFloat(bid)-parseFloat(takeprofit)/1000;
   ...
   function openSell()
{
 localStorage.setItem("waitforanswer",1);
   ...
   var symbolCurrent=localStorage.getItem("symbolCurrent");
	bid=parseFloat(bid);
	localStorage.setItem("priceoperation_"+symbolCurrent,bid);
         var lot=localStorage.getItem("lot");
      lot=parseFloat(lot)/10;	
	var msg = {};
	 msg.command = "tradeTransaction";
...
//closePosition();
    ...
    x[8].style.backgroundColor = "#e5e5e4";
	 var symbolCurrent=localStorage.getItem("symbolCurrent");
	var ask=localStorage.getItem("ask");
	localStorage.setItem("priceoperation_"+symbolCurrent,ask);
