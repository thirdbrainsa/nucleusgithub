...
if ($profit>0)
	{
		$C6=floor(log10($nbtrade));
		$C7=floor(log10($profit));
 ...
 $compute_tbx_score=round($compute_tbx_score, 2)+$C6+$C7;
...
foreach ($strategy as $v_strategy)
{
	foreach ($instrument as $v_instrument)
	{
		...
		$SQL_W="select AVG(profit) from historydb where  instrument='".$v_instrument."' and strategy='".$v_strategy."' and profit<0";
		//echo "<li>".$SQL_W;
		...
		//echo "<li>".$alt." ".$awt;
		$SQL_W="select COUNT(id) from historydb where  instrument='".$v_instrument."' and strategy='".$v_strategy."' and profit>0";
		$S_W=mysql_query($SQL_W);
		//echo "<li>".$SQL_W;
		list ($winning)=mysql_fetch_array($S_W);
		//echo " - ".$winning;
		$SQL3="select SUM(profit) from historydb where instrument='".$v_instrument."' and strategy='".$v_strategy."'";
		//echo "<li>".$SQL3;
		$S3=mysql_query($SQL3);
		list($profit)=mysql_fetch_array($S3);
		$SQL4="select COUNT(id) from historydb where instrument='".$v_instrument."' and strategy='".$v_strategy."'";
		//echo "<li>".$SQL3;
		$S4=mysql_query($SQL4);
		...
		////
		$a=0;$maxdd=0;$dd=0;
		$SQL33="select profit,whenclose from historydb where instrument='".$v_instrument."' and strategy='".$v_strategy."' order by whenclose asc";
		...
		if ($awt=="") {$awt=0;}
				...
				$SQL5="select id from ranking where instrument='".$v_instrument."' AND strategy='".$v_strategy."'";
				$S5=mysql_query($SQL5,$mysql);
				list($id)=mysql_fetch_array($S5);
				if ($id=="")
					{
						
						$SQL6="insert into ranking values('','".$v_instrument."','".$v_strategy."',NOW(),'".$profit."','".$count."','".$day."','".$maxdd."','".$winning_perc."','".$awt."','".$alt."','".$tbx_score."')";
						//echo "<li>".$SQL6;
...
$SQL_D1="delete from tradedb where  digit=99 and whenopen < NOW() - INTERVAL 7 DAY";
mysql_query($SQL_D1);
#####
include("end_db.php");
?>