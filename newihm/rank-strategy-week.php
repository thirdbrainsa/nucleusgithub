...
$SQL_D1="TRUNCATE ranking_week";
mysql_query($SQL_D1);

$SQL1="SELECT DISTINCT(strategy) from historydb where whenclose>NOW() - INTERVAL 1  DAY";
$SQL2="SELECT DISTINCT(instrument) from historydb where whenclose>NOW() - INTERVAL 1  DAY";
...
foreach ($strategy as $v_strategy)
{
	foreach ($instrument as $v_instrument)
	{
		...
		$SQL_W="select AVG(profit) from historydb where  instrument='".$v_instrument."' and strategy='".$v_strategy."' and profit<0 and whenclose>NOW() - INTERVAL 1  DAY";
		//echo "<li>".$SQL_W;
		...
		//echo "<li>".$alt." ".$awt;
		$SQL_W="select COUNT(id) from historydb where  instrument='".$v_instrument."' and strategy='".$v_strategy."' and profit>0 and whenclose>NOW() - INTERVAL 1  DAY";
		$S_W=mysql_query($SQL_W);
		//echo "<li>".$SQL_W;
		list ($winning)=mysql_fetch_array($S_W);
		//echo " - ".$winning;
		$SQL3="select SUM(profit) from historydb where instrument='".$v_instrument."' and strategy='".$v_strategy."' and whenclose>NOW() - INTERVAL 1  DAY";
		//echo "<li>".$SQL3;
		$S3=mysql_query($SQL3);
		list($profit)=mysql_fetch_array($S3);
		$SQL4="select COUNT(id) from historydb where instrument='".$v_instrument."' and strategy='".$v_strategy."' and whenclose>NOW() - INTERVAL 1  DAY";
		//echo "<li>".$SQL3;
		$S4=mysql_query($SQL4);
		...
		$SQL5="select SUM(profit) from historydb where instrument='".$v_instrument."' and strategy='".$v_strategy."' and whenclose > NOW() - INTERVAL 1 DAY";
		$S5=mysql_query($SQL5);
		...
		////
		$a=0;$maxdd=0;$dd=0;
		$SQL33="select profit,whenclose from historydb where instrument='".$v_instrument."' and strategy='".$v_strategy."' and whenclose>NOW() - INTERVAL 1  DAY order by whenclose asc";
		...
		if ($awt=="") {$awt=0;}
				...
				$SQL5="select id from ranking_week where instrument='".$v_instrument."' AND strategy='".$v_strategy."'";
				$S5=mysql_query($SQL5,$mysql);
				list($id)=mysql_fetch_array($S5);
				if ($id=="")
					{
						
						$SQL6="insert into ranking_week values('','".$v_instrument."','".$v_strategy."',NOW(),'".$profit."','".$count."','".$day."','".$maxdd."','".$winning_perc."','".$awt."','".$alt."','".$tbx_score."')";
						//echo "<li>".$SQL6;
