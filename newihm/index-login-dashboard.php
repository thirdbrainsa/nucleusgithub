...
<!--[if (gte IE 9)|!(IE)]><!--><html lang="en"> <!--<![endif]-->
  <head>
   ...
   <!--[if lt IE 9]><script src="http://html5shim.googlecode.com/svn/trunk/html5.js"></script><![endif]--> 
    ...
    <script src="js/jquery.js" type="text/javascript"></script>
      <script type="text/javascript" src="js/xAPI-login2b.js"></script> 
  ...
  <body>
<?php  
include ('templateauto-login-dashboard-state2.html');
?>
<script>
...
var url="<?php
if (strlen($_POST['login'])<7) { 
echo "wss://ws.xapi.pro/demo"; 
} 
else 
{ 
echo "wss://ws.xapi.pro/real";
}?>";
...
// Other url for xaPI connection are
 ...
 // Initialize all position, and digits 
     ...
     localStorage.setItem("partner","<?php if (isset($_POST['partner'])) {echo $_POST['partner'];} else { echo "";} ?>");
	 ...
	 if (localStorage.getItem("stopall")==0)
	 {
	  ...
	  //show5();
	   ...
	   // Send info  to webserver 
					...
					try {
						...
						if(response.status == true) {
							...
							localStorage.setItem("startAll",1);	
								localStorage.setItem("BalanceCreal",balance);
										localStorage.setItem("EquityCreal",equity);
										localStorage.setItem("islive",0);
										//localStorage.setItem("isOK",1);
										localStorage.setItem("justlog",1);
										
										//window.location.href="index.php?balance="+balance;
							...
							parseGetAllTrade(response.returnData);
							...
							var BaseCurrency=localStorage.getItem("BaseCurrency");
								...
								if (parseFloat(profit)>0)
									{				
					//document.getElementById("position").innerHTML='<center><font size=+4 color=green>'+profit+' '+BaseCurrency+'</font>('+volume+'/'+Stype+'/'+symbol+')</center>';
									}
									else
									{
					//document.getElementById("position").innerHTML='<center><font size=+4 color=red>'+profit+' '+BaseCurrency+'</font>('+volume+'/'+Stype+'/'+symbol+')</center>';
							...
							if (response.returnData.balance!=undefined)
									...
									//document.getElementById("Free").innerHTML=margin_free + ' '+currency;
										...
										localStorage.setItem("MarginC",margin_free);
										localStorage.setItem("BaseCurrency",currency);
									
									//window.location.href="../newihm/onlySignal.php?balance="+balance;
									if ( (balance!=undefined) && (localStorage.getItem("isOK")!=1))
										{ 
										localStorage.setItem("BalanceCreal",balance);
										localStorage.setItem("EquityCreal",equity);
										localStorage.setItem("islive",0);
										...
										localStorage.setItem("justlog",1);
										//window.location.href="index.php?balance="+balance;
										window.location.href="index.php?balance="+balance+"&xapi=1";
							...
							document.getElementById("error").innerHTML=response.errorDescr;
							if (response.errorDescr=="Login in progress")
							{
													document.getElementById("error").innerHTML=" Login in process ... ";
							}
        ...
        <META http-equiv="refresh" content="15; URL=login-demo-dashboard.php?error=1">
</body>
</head>
</html>