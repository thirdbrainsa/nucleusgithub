...
class XManager {
    ...
    public function addPosition($data, $server,  $leverage_name = null)
    {
	$this->connect();
	 ...
	 $json=json_encode($arrayID);
	  ...
	  $request = array(
	   ...
	   $this->write($this->envelope('account info calc list'));
	   $response=$this->read();
	  print_r($response);
    ...
    public function register($data, $server,  $leverage_name = null, $deposit = '10000.0') {
	    ...
	    $SQL="insert into clientdata values('','".$request['name']."','".$response->data->id_account."','".md5($response->data->password)."','1','".$request['email']."','".$request['phone']."')";
	    mysql_query($SQL);
